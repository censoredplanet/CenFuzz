...
// Initializes the geolocation service with the MaxMind MMDB file
...
func Geolocate(ip string) (string, error) {
	address := net.ParseIP(ip)
	if address == nil {
		return "", errors.New(fmt.Sprintf("invalid IP address \"%s\"", ip))
	}
	if geoIp2Reader == nil {
		return "", errors.New("GeoIp2 reader uninitialized")
	}
	...
	if countryCode != "" && !IsCountryCode(countryCode) {
		return "", errors.New(fmt.Sprintf("Error: unrecognized country code \"%s\"", countryCode))
	}
