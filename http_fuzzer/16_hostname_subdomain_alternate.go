...
func (h *HostnameSubdomainsAlternate) Init(all bool) []*RequestWord {
	var requestWords []*RequestWord
	var requestWord *RequestWord
	retries := 0
	if !all {
		for i := 0; i < config.NumberOfProbesPerTest; i++ {
			HostnameAlternate := util.GenerateSubdomainsAlternatives()
			...
			if containsRequestWord(requestWords, requestWord) {
				...
				retries += 1
				if retries >= 10 {
					log.Println("[HostnameSubdomainsAlternate.Init] Could not find a new random value after 10 retries. Breaking.")
					break
				}
			} else {
		...
		HostnameAllAlternatives := util.GenerateAllSubdomainsAlternatives()
